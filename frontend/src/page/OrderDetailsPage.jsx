...
const OrderDetailsPage = () => {
  ...
  const handleCancelOrder = () => {
    if (window.confirm("Are you sure you want to cancel this order?")) {
      dispatch(updateOrderStatus({ orderId: id, status: "Canceled" })).then(
        () => {
          dispatch(fetchOrderDetails(id));
        }
      );
  ...
  const renderStatus = (status) => {
    switch (status) {
      ...
      case "Processing":
        return (
          <span className="bg-yellow-100 text-yellow-700 px-3 py-1 rounded-full text-sm font-medium mb-2">
            Processing
          </span>
        );
      case "Shipped":
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6">
      ...
      <div className="p-4 sm:p-6 rounded-lg border">
        ...
        {/* Discount */}
        <div className="mb-4">
          <h4 className="text-lg font-semibold mb-2">Discount</h4>
          {orderDetails.coupon ? (
            <>
              <p>
                Coupon Code:{" "}
                <span className="font-medium">{orderDetails.coupon}</span>
              </p>
              <p>
                Discount Amount: $
